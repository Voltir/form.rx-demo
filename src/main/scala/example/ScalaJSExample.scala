...
import formidable.ImplicitsNext._
import Framework._
...
//  }
    ...
    row(
      ...
      column("small-9")(
        formTag(
          input(`type`:="Submit"),
          onsubmit := {() =>
            println(formidable.current.now)
            false
          }
        )
      )
  ...
  val firstRx: HtmlTag = {
    import Demo1._
    val form1  = FormidableRx[Demo1.UserPassLayout,Demo1.UserPass]
    val default = Demo1.UserPass("Bob!","supersecretbob")
    templateRx("Example 1","Basic User/Password form")(form1,"Bob",default) {
      form(
        form1.firstname,
        form1.pass
      )
  ...
  def secondRx: HtmlTag = {
    ...
    val default = Nested("This is top",Inner("This is foo",2),Inner("Other foo",5))
    templateRx("Example 2", "Formidable can nest")(form2,"Default",default) {
      form(
        form2.top,
        label("Inner:"),
        form2.inner.foo,
  ...
  val imgRx: HtmlTag = {
    ...
    templateRx("Example Img Upload","Basic User/Password form")(imgForm,"Wurt",default) {
  ...
  def main(content: dom.html.Div): Unit = {
    ...
    content.appendChild(Seq(firstRx,hr).render)
    content.appendChild(Seq(secondRx,hr).render)
